import { LevelDefinition } from '@centerhit-features/levels/types/levelTypes';

export const levels: readonly LevelDefinition[] = [
  {
    id: 'level-01',
    order: 1,
    title: 'Soft Launch',
    difficulty: 'easy',
    launcher: { speed: 1.2, moveRangePercent: 0.16 },
    target: {
      size: 1,
      speed: 0.6,
      movementAxis: 'static',
      movementBehavior: 'bounce',
      moveRangePercent: 0,
    },
    ball: { speed: 1 },
    objective: { requiredHits: 4, requiredPerfectHits: 0, maxMissAllowed: 4, maxShots: 6 },
    stars: { oneStarScore: 200, twoStarScore: 320, threeStarScore: 440 },
  },
  {
    id: 'level-02',
    order: 2,
    title: 'Pulse Center',
    difficulty: 'easy',
    launcher: { speed: 1.34, moveRangePercent: 0.2 },
    target: {
      size: 0.9,
      speed: 0.8,
      movementAxis: 'static',
      movementBehavior: 'bounce',
      moveRangePercent: 0,
    },
    ball: { speed: 1.05 },
    objective: { requiredHits: 5, requiredPerfectHits: 1, maxMissAllowed: 3, maxShots: 7 },
    stars: { oneStarScore: 280, twoStarScore: 420, threeStarScore: 560 },
  },
  {
    id: 'level-03',
    order: 3,
    title: 'Opening Drift',
    difficulty: 'medium',
    launcher: { speed: 1.48, moveRangePercent: 0.24 },
    target: {
      size: 0.84,
      speed: 0.94,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.2,
    },
    ball: { speed: 1.1 },
    objective: { requiredHits: 6, requiredPerfectHits: 1, maxMissAllowed: 3, maxShots: 8 },
    stars: { oneStarScore: 360, twoStarScore: 500, threeStarScore: 660 },
  },
  {
    id: 'level-04',
    order: 4,
    title: 'Sync Line',
    difficulty: 'medium',
    launcher: { speed: 1.56, moveRangePercent: 0.26 },
    target: {
      size: 0.78,
      speed: 1.08,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.26,
    },
    ball: { speed: 1.12 },
    objective: { requiredHits: 6, requiredPerfectHits: 2, maxMissAllowed: 3, maxShots: 8 },
    stars: { oneStarScore: 390, twoStarScore: 560, threeStarScore: 700 },
  },
  {
    id: 'level-05',
    order: 5,
    title: 'Sweep Window',
    difficulty: 'medium',
    launcher: { speed: 1.68, moveRangePercent: 0.28 },
    target: {
      size: 0.72,
      speed: 1.2,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.3,
    },
    ball: { speed: 1.14 },
    objective: { requiredHits: 7, requiredPerfectHits: 2, maxMissAllowed: 2, maxShots: 10 },
    stars: { oneStarScore: 470, twoStarScore: 650, threeStarScore: 800 },
    obstacles: [
      {
        id: 'obs-05-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.57,
        widthPercent: 0.11,
        heightPercent: 0.038,
      },
    ],
  },
  {
    id: 'level-06',
    order: 6,
    title: 'Skill Gate',
    difficulty: 'medium',
    launcher: { speed: 1.78, moveRangePercent: 0.3 },
    target: {
      size: 0.67,
      speed: 1.32,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.34,
    },
    ball: { speed: 1.18 },
    objective: { requiredHits: 8, requiredPerfectHits: 3, maxMissAllowed: 2, maxShots: 10 },
    stars: { oneStarScore: 540, twoStarScore: 740, threeStarScore: 900 },
    obstacles: [
      {
        id: 'obs-06-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.54,
        widthPercent: 0.14,
        heightPercent: 0.042,
      },
    ],
  },
  {
    id: 'level-07',
    order: 7,
    title: 'Rhythm Break',
    difficulty: 'hard',
    launcher: { speed: 1.86, moveRangePercent: 0.31 },
    target: {
      size: 0.62,
      speed: 1.42,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.34,
    },
    ball: { speed: 1.22 },
    objective: { requiredHits: 8, requiredPerfectHits: 3, maxMissAllowed: 2, maxShots: 11 },
    stars: { oneStarScore: 590, twoStarScore: 780, threeStarScore: 930 },
    obstacles: [
      {
        id: 'obs-07-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.56,
        widthPercent: 0.09,
        heightPercent: 0.036,
      },
    ],
  },
  {
    id: 'level-08',
    order: 8,
    title: 'Tight Relay',
    difficulty: 'hard',
    launcher: { speed: 1.98, moveRangePercent: 0.34 },
    target: {
      size: 0.58,
      speed: 1.56,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.36,
    },
    ball: { speed: 1.26 },
    objective: { requiredHits: 9, requiredPerfectHits: 4, maxMissAllowed: 2, maxShots: 12 },
    stars: { oneStarScore: 660, twoStarScore: 840, threeStarScore: 1020 },
    obstacles: [
      {
        id: 'obs-08-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.55,
        widthPercent: 0.11,
        heightPercent: 0.04,
      },
    ],
  },
  {
    id: 'level-09',
    order: 9,
    title: 'Split View',
    difficulty: 'hard',
    launcher: { speed: 2.06, moveRangePercent: 0.36 },
    target: {
      size: 0.54,
      speed: 1.64,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.38,
    },
    ball: { speed: 1.3 },
    objective: { requiredHits: 10, requiredPerfectHits: 4, maxMissAllowed: 2, maxShots: 12 },
    stars: { oneStarScore: 720, twoStarScore: 920, threeStarScore: 1100 },
    obstacles: [
      {
        id: 'obs-09-a',
        type: 'blocker',
        xPercent: 0.34,
        yPercent: 0.5,
        widthPercent: 0.14,
        heightPercent: 0.04,
      },
      {
        id: 'obs-09-b',
        type: 'blocker',
        xPercent: 0.66,
        yPercent: 0.5,
        widthPercent: 0.14,
        heightPercent: 0.04,
      },
    ],
  },
  {
    id: 'level-10',
    order: 10,
    title: 'Crown Gate',
    difficulty: 'hard',
    launcher: { speed: 2.16, moveRangePercent: 0.38 },
    target: {
      size: 0.48,
      speed: 1.78,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.4,
    },
    ball: { speed: 1.32 },
    objective: { requiredHits: 11, requiredPerfectHits: 5, maxMissAllowed: 1, maxShots: 13 },
    stars: { oneStarScore: 800, twoStarScore: 1040, threeStarScore: 1240 },
    obstacles: [
      {
        id: 'obs-10-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.52,
        widthPercent: 0.1,
        heightPercent: 0.04,
      },
      {
        id: 'obs-10-b',
        type: 'blocker',
        xPercent: 0.28,
        yPercent: 0.45,
        widthPercent: 0.09,
        heightPercent: 0.036,
      },
      {
        id: 'obs-10-c',
        type: 'blocker',
        xPercent: 0.72,
        yPercent: 0.45,
        widthPercent: 0.09,
        heightPercent: 0.036,
      },
    ],
  },
  {
    id: 'level-11',
    order: 11,
    title: 'Neon Lock',
    difficulty: 'hard',
    launcher: { speed: 2.22, moveRangePercent: 0.4 },
    target: {
      size: 0.46,
      speed: 1.82,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.42,
    },
    ball: { speed: 1.34 },
    objective: { requiredHits: 11, requiredPerfectHits: 5, maxMissAllowed: 1, maxShots: 13 },
    stars: { oneStarScore: 820, twoStarScore: 1060, threeStarScore: 1260 },
    obstacles: [
      {
        id: 'obs-11-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.52,
        widthPercent: 0.1,
        heightPercent: 0.038,
      },
    ],
  },
  {
    id: 'level-12',
    order: 12,
    title: 'Twin Drift',
    difficulty: 'hard',
    launcher: { speed: 2.28, moveRangePercent: 0.41 },
    target: {
      size: 0.45,
      speed: 1.88,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.43,
    },
    ball: { speed: 1.35 },
    objective: { requiredHits: 12, requiredPerfectHits: 5, maxMissAllowed: 1, maxShots: 14 },
    stars: { oneStarScore: 860, twoStarScore: 1100, threeStarScore: 1300 },
    obstacles: [
      {
        id: 'obs-12-a',
        type: 'blocker',
        xPercent: 0.35,
        yPercent: 0.5,
        widthPercent: 0.1,
        heightPercent: 0.036,
      },
      {
        id: 'obs-12-b',
        type: 'blocker',
        xPercent: 0.65,
        yPercent: 0.5,
        widthPercent: 0.1,
        heightPercent: 0.036,
      },
    ],
  },
  {
    id: 'level-13',
    order: 13,
    title: 'Gate Pulse',
    difficulty: 'hard',
    launcher: { speed: 2.34, moveRangePercent: 0.42 },
    target: {
      size: 0.43,
      speed: 1.94,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.44,
    },
    ball: { speed: 1.36 },
    objective: { requiredHits: 12, requiredPerfectHits: 6, maxMissAllowed: 1, maxShots: 14 },
    stars: { oneStarScore: 900, twoStarScore: 1150, threeStarScore: 1350 },
    obstacles: [
      {
        id: 'obs-13-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.54,
        widthPercent: 0.09,
        heightPercent: 0.036,
      },
      {
        id: 'obs-13-b',
        type: 'blocker',
        xPercent: 0.28,
        yPercent: 0.44,
        widthPercent: 0.08,
        heightPercent: 0.034,
      },
      {
        id: 'obs-13-c',
        type: 'blocker',
        xPercent: 0.72,
        yPercent: 0.44,
        widthPercent: 0.08,
        heightPercent: 0.034,
      },
    ],
  },
  {
    id: 'level-14',
    order: 14,
    title: 'Cross Current',
    difficulty: 'hard',
    launcher: { speed: 2.38, moveRangePercent: 0.43 },
    target: {
      size: 0.42,
      speed: 1.98,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.45,
    },
    ball: { speed: 1.37 },
    objective: { requiredHits: 12, requiredPerfectHits: 6, maxMissAllowed: 1, maxShots: 15 },
    stars: { oneStarScore: 940, twoStarScore: 1190, threeStarScore: 1390 },
    obstacles: [
      {
        id: 'obs-14-a',
        type: 'blocker',
        xPercent: 0.22,
        yPercent: 0.48,
        widthPercent: 0.08,
        heightPercent: 0.034,
      },
      {
        id: 'obs-14-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.56,
        widthPercent: 0.09,
        heightPercent: 0.034,
      },
      {
        id: 'obs-14-c',
        type: 'blocker',
        xPercent: 0.78,
        yPercent: 0.48,
        widthPercent: 0.08,
        heightPercent: 0.034,
      },
    ],
  },
  {
    id: 'level-15',
    order: 15,
    title: 'Echo Lanes',
    difficulty: 'hard',
    launcher: { speed: 2.42, moveRangePercent: 0.44 },
    target: {
      size: 0.41,
      speed: 2.02,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.46,
    },
    ball: { speed: 1.38 },
    objective: { requiredHits: 13, requiredPerfectHits: 6, maxMissAllowed: 1, maxShots: 15 },
    stars: { oneStarScore: 980, twoStarScore: 1230, threeStarScore: 1430 },
    obstacles: [
      {
        id: 'obs-15-a',
        type: 'blocker',
        xPercent: 0.34,
        yPercent: 0.52,
        widthPercent: 0.09,
        heightPercent: 0.034,
      },
      {
        id: 'obs-15-b',
        type: 'blocker',
        xPercent: 0.66,
        yPercent: 0.52,
        widthPercent: 0.09,
        heightPercent: 0.034,
      },
    ],
  },
  {
    id: 'level-16',
    order: 16,
    title: 'Signal Cut',
    difficulty: 'hard',
    launcher: { speed: 2.46, moveRangePercent: 0.45 },
    target: {
      size: 0.39,
      speed: 2.08,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.47,
    },
    ball: { speed: 1.39 },
    objective: { requiredHits: 13, requiredPerfectHits: 7, maxMissAllowed: 1, maxShots: 15 },
    stars: { oneStarScore: 1020, twoStarScore: 1270, threeStarScore: 1470 },
    obstacles: [
      {
        id: 'obs-16-a',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.48,
        widthPercent: 0.08,
        heightPercent: 0.032,
      },
      {
        id: 'obs-16-b',
        type: 'blocker',
        xPercent: 0.28,
        yPercent: 0.58,
        widthPercent: 0.08,
        heightPercent: 0.032,
      },
      {
        id: 'obs-16-c',
        type: 'blocker',
        xPercent: 0.72,
        yPercent: 0.58,
        widthPercent: 0.08,
        heightPercent: 0.032,
      },
    ],
  },
  {
    id: 'level-17',
    order: 17,
    title: 'Flash Arc',
    difficulty: 'hard',
    launcher: { speed: 2.5, moveRangePercent: 0.46 },
    target: {
      size: 0.38,
      speed: 2.12,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.48,
    },
    ball: { speed: 1.4 },
    objective: { requiredHits: 14, requiredPerfectHits: 7, maxMissAllowed: 1, maxShots: 16 },
    stars: { oneStarScore: 1060, twoStarScore: 1310, threeStarScore: 1510 },
    obstacles: [
      {
        id: 'obs-17-a',
        type: 'blocker',
        xPercent: 0.18,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.03,
      },
      {
        id: 'obs-17-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.56,
        widthPercent: 0.08,
        heightPercent: 0.03,
      },
      {
        id: 'obs-17-c',
        type: 'blocker',
        xPercent: 0.82,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.03,
      },
    ],
  },
  {
    id: 'level-18',
    order: 18,
    title: 'Narrow Shift',
    difficulty: 'hard',
    launcher: { speed: 2.54, moveRangePercent: 0.47 },
    target: {
      size: 0.37,
      speed: 2.18,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.49,
    },
    ball: { speed: 1.41 },
    objective: { requiredHits: 14, requiredPerfectHits: 8, maxMissAllowed: 1, maxShots: 16 },
    stars: { oneStarScore: 1100, twoStarScore: 1350, threeStarScore: 1550 },
    obstacles: [
      {
        id: 'obs-18-a',
        type: 'blocker',
        xPercent: 0.36,
        yPercent: 0.46,
        widthPercent: 0.08,
        heightPercent: 0.03,
      },
      {
        id: 'obs-18-b',
        type: 'blocker',
        xPercent: 0.64,
        yPercent: 0.58,
        widthPercent: 0.08,
        heightPercent: 0.03,
      },
    ],
  },
  {
    id: 'level-19',
    order: 19,
    title: 'Grid Shock',
    difficulty: 'hard',
    launcher: { speed: 2.58, moveRangePercent: 0.48 },
    target: {
      size: 0.36,
      speed: 2.22,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.5,
    },
    ball: { speed: 1.42 },
    objective: { requiredHits: 15, requiredPerfectHits: 8, maxMissAllowed: 1, maxShots: 16 },
    stars: { oneStarScore: 1140, twoStarScore: 1390, threeStarScore: 1590 },
    obstacles: [
      {
        id: 'obs-19-a',
        type: 'blocker',
        xPercent: 0.26,
        yPercent: 0.52,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
      {
        id: 'obs-19-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.44,
        widthPercent: 0.08,
        heightPercent: 0.03,
      },
      {
        id: 'obs-19-c',
        type: 'blocker',
        xPercent: 0.74,
        yPercent: 0.52,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
    ],
  },
  {
    id: 'level-20',
    order: 20,
    title: 'Final Drift',
    difficulty: 'hard',
    launcher: { speed: 2.62, moveRangePercent: 0.49 },
    target: {
      size: 0.35,
      speed: 2.26,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.5,
    },
    ball: { speed: 1.43 },
    objective: { requiredHits: 15, requiredPerfectHits: 9, maxMissAllowed: 1, maxShots: 17 },
    stars: { oneStarScore: 1180, twoStarScore: 1430, threeStarScore: 1630 },
    obstacles: [
      {
        id: 'obs-20-a',
        type: 'blocker',
        xPercent: 0.2,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
      {
        id: 'obs-20-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.58,
        widthPercent: 0.08,
        heightPercent: 0.03,
      },
      {
        id: 'obs-20-c',
        type: 'blocker',
        xPercent: 0.8,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
    ],
  },
  {
    id: 'level-21',
    order: 21,
    title: 'Signal Crown',
    difficulty: 'hard',
    launcher: { speed: 2.66, moveRangePercent: 0.5 },
    target: {
      size: 0.34,
      speed: 2.3,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.52,
    },
    ball: { speed: 1.44 },
    objective: { requiredHits: 15, requiredPerfectHits: 9, maxMissAllowed: 1, maxShots: 17 },
    stars: { oneStarScore: 1220, twoStarScore: 1470, threeStarScore: 1670 },
    obstacles: [
      {
        id: 'obs-21-a',
        type: 'blocker',
        xPercent: 0.34,
        yPercent: 0.46,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
      {
        id: 'obs-21-b',
        type: 'blocker',
        xPercent: 0.66,
        yPercent: 0.58,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
    ],
  },
  {
    id: 'level-22',
    order: 22,
    title: 'Core Mirror',
    difficulty: 'hard',
    launcher: { speed: 2.7, moveRangePercent: 0.51 },
    target: {
      size: 0.33,
      speed: 2.34,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.52,
    },
    ball: { speed: 1.45 },
    objective: { requiredHits: 16, requiredPerfectHits: 9, maxMissAllowed: 1, maxShots: 17 },
    stars: { oneStarScore: 1260, twoStarScore: 1510, threeStarScore: 1710 },
    obstacles: [
      {
        id: 'obs-22-a',
        type: 'blocker',
        xPercent: 0.24,
        yPercent: 0.54,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
      {
        id: 'obs-22-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.46,
        widthPercent: 0.08,
        heightPercent: 0.028,
      },
      {
        id: 'obs-22-c',
        type: 'blocker',
        xPercent: 0.76,
        yPercent: 0.54,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
    ],
  },
  {
    id: 'level-23',
    order: 23,
    title: 'Split Pulse',
    difficulty: 'hard',
    launcher: { speed: 2.74, moveRangePercent: 0.52 },
    target: {
      size: 0.32,
      speed: 2.38,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.53,
    },
    ball: { speed: 1.46 },
    objective: { requiredHits: 16, requiredPerfectHits: 10, maxMissAllowed: 1, maxShots: 18 },
    stars: { oneStarScore: 1300, twoStarScore: 1550, threeStarScore: 1750 },
    obstacles: [
      {
        id: 'obs-23-a',
        type: 'blocker',
        xPercent: 0.4,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.026,
      },
      {
        id: 'obs-23-b',
        type: 'blocker',
        xPercent: 0.6,
        yPercent: 0.5,
        widthPercent: 0.07,
        heightPercent: 0.026,
      },
    ],
  },
  {
    id: 'level-24',
    order: 24,
    title: 'Echo Crown',
    difficulty: 'hard',
    launcher: { speed: 2.78, moveRangePercent: 0.53 },
    target: {
      size: 0.31,
      speed: 2.42,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.54,
    },
    ball: { speed: 1.47 },
    objective: { requiredHits: 17, requiredPerfectHits: 10, maxMissAllowed: 1, maxShots: 18 },
    stars: { oneStarScore: 1340, twoStarScore: 1590, threeStarScore: 1790 },
    obstacles: [
      {
        id: 'obs-24-a',
        type: 'blocker',
        xPercent: 0.18,
        yPercent: 0.48,
        widthPercent: 0.06,
        heightPercent: 0.026,
      },
      {
        id: 'obs-24-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.56,
        widthPercent: 0.07,
        heightPercent: 0.028,
      },
      {
        id: 'obs-24-c',
        type: 'blocker',
        xPercent: 0.82,
        yPercent: 0.48,
        widthPercent: 0.06,
        heightPercent: 0.026,
      },
    ],
  },
  {
    id: 'level-25',
    order: 25,
    title: 'Static Fracture',
    difficulty: 'hard',
    launcher: { speed: 2.82, moveRangePercent: 0.54 },
    target: {
      size: 0.3,
      speed: 2.46,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.55,
    },
    ball: { speed: 1.48 },
    objective: { requiredHits: 17, requiredPerfectHits: 10, maxMissAllowed: 1, maxShots: 18 },
    stars: { oneStarScore: 1380, twoStarScore: 1630, threeStarScore: 1830 },
    obstacles: [
      {
        id: 'obs-25-a',
        type: 'blocker',
        xPercent: 0.3,
        yPercent: 0.44,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
      {
        id: 'obs-25-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.6,
        widthPercent: 0.07,
        heightPercent: 0.026,
      },
      {
        id: 'obs-25-c',
        type: 'blocker',
        xPercent: 0.7,
        yPercent: 0.44,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
    ],
  },
  {
    id: 'level-26',
    order: 26,
    title: 'Pulse Rift',
    difficulty: 'hard',
    launcher: { speed: 2.86, moveRangePercent: 0.55 },
    target: {
      size: 0.29,
      speed: 2.5,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.56,
    },
    ball: { speed: 1.49 },
    objective: { requiredHits: 18, requiredPerfectHits: 11, maxMissAllowed: 1, maxShots: 19 },
    stars: { oneStarScore: 1420, twoStarScore: 1670, threeStarScore: 1870 },
    obstacles: [
      {
        id: 'obs-26-a',
        type: 'blocker',
        xPercent: 0.22,
        yPercent: 0.52,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
      {
        id: 'obs-26-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.48,
        widthPercent: 0.07,
        heightPercent: 0.026,
      },
      {
        id: 'obs-26-c',
        type: 'blocker',
        xPercent: 0.78,
        yPercent: 0.52,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
    ],
  },
  {
    id: 'level-27',
    order: 27,
    title: 'Neon Spine',
    difficulty: 'hard',
    launcher: { speed: 2.9, moveRangePercent: 0.56 },
    target: {
      size: 0.28,
      speed: 2.54,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.57,
    },
    ball: { speed: 1.5 },
    objective: { requiredHits: 18, requiredPerfectHits: 11, maxMissAllowed: 1, maxShots: 19 },
    stars: { oneStarScore: 1460, twoStarScore: 1710, threeStarScore: 1910 },
    obstacles: [
      {
        id: 'obs-27-a',
        type: 'blocker',
        xPercent: 0.18,
        yPercent: 0.46,
        widthPercent: 0.055,
        heightPercent: 0.024,
      },
      {
        id: 'obs-27-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.56,
        widthPercent: 0.065,
        heightPercent: 0.026,
      },
      {
        id: 'obs-27-c',
        type: 'blocker',
        xPercent: 0.82,
        yPercent: 0.46,
        widthPercent: 0.055,
        heightPercent: 0.024,
      },
    ],
  },
  {
    id: 'level-28',
    order: 28,
    title: 'Hardline Flow',
    difficulty: 'hard',
    launcher: { speed: 2.94, moveRangePercent: 0.57 },
    target: {
      size: 0.27,
      speed: 2.58,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.58,
    },
    ball: { speed: 1.51 },
    objective: { requiredHits: 19, requiredPerfectHits: 12, maxMissAllowed: 1, maxShots: 19 },
    stars: { oneStarScore: 1500, twoStarScore: 1750, threeStarScore: 1950 },
    obstacles: [
      {
        id: 'obs-28-a',
        type: 'blocker',
        xPercent: 0.34,
        yPercent: 0.48,
        widthPercent: 0.055,
        heightPercent: 0.022,
      },
      {
        id: 'obs-28-b',
        type: 'blocker',
        xPercent: 0.66,
        yPercent: 0.58,
        widthPercent: 0.055,
        heightPercent: 0.022,
      },
    ],
  },
  {
    id: 'level-29',
    order: 29,
    title: 'Edge Pulse',
    difficulty: 'hard',
    launcher: { speed: 2.98, moveRangePercent: 0.58 },
    target: {
      size: 0.26,
      speed: 2.62,
      movementAxis: 'horizontal',
      movementBehavior: 'loop',
      moveRangePercent: 0.59,
    },
    ball: { speed: 1.52 },
    objective: { requiredHits: 19, requiredPerfectHits: 12, maxMissAllowed: 1, maxShots: 20 },
    stars: { oneStarScore: 1540, twoStarScore: 1790, threeStarScore: 1990 },
    obstacles: [
      {
        id: 'obs-29-a',
        type: 'blocker',
        xPercent: 0.24,
        yPercent: 0.56,
        widthPercent: 0.055,
        heightPercent: 0.022,
      },
      {
        id: 'obs-29-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.46,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
      {
        id: 'obs-29-c',
        type: 'blocker',
        xPercent: 0.76,
        yPercent: 0.56,
        widthPercent: 0.055,
        heightPercent: 0.022,
      },
    ],
  },
  {
    id: 'level-30',
    order: 30,
    title: 'Summit Gate',
    difficulty: 'hard',
    launcher: { speed: 3.02, moveRangePercent: 0.6 },
    target: {
      size: 0.25,
      speed: 2.68,
      movementAxis: 'horizontal',
      movementBehavior: 'bounce',
      moveRangePercent: 0.6,
    },
    ball: { speed: 1.54 },
    objective: { requiredHits: 20, requiredPerfectHits: 12, maxMissAllowed: 1, maxShots: 20 },
    stars: { oneStarScore: 1600, twoStarScore: 1850, threeStarScore: 2050 },
    obstacles: [
      {
        id: 'obs-30-a',
        type: 'blocker',
        xPercent: 0.2,
        yPercent: 0.5,
        widthPercent: 0.05,
        heightPercent: 0.022,
      },
      {
        id: 'obs-30-b',
        type: 'blocker',
        xPercent: 0.5,
        yPercent: 0.58,
        widthPercent: 0.06,
        heightPercent: 0.024,
      },
      {
        id: 'obs-30-c',
        type: 'blocker',
        xPercent: 0.8,
        yPercent: 0.5,
        widthPercent: 0.05,
        heightPercent: 0.022,
      },
    ],
  },
] as const;
